...
namespace Automation.BrowserFolder
{
    public class HubLoadBalancer
    {
        ...
        public HubLoadBalancer(Configurations config)
        {
            ...
            _hub1 = _apiObj.GetRequest($"http://ip-10-0-8-224.us-west-2.compute.internal/grid/api/hub")["slotCounts"]["free"].ToString();
            _hub2 = _apiObj.GetRequest($"http://ip-10-0-6-142.us-west-2.compute.internal/grid/api/hub")["slotCounts"]["free"].ToString();
        }
